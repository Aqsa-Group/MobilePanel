<div class="w-full p-4 max-w-full mx-auto">
    <div class="rounded-xl shadow-xl w-full shadow-[0px_4px_4px_0px_#00000040] bg-[#616161]/5 px-2 py-4">
        <h2 class="text-2xl text-center font-bold mb-2">کارمند جدید</h2>
        <p class="text-gray-500 text-center text-sm">شما می‌توانید کارمند جدید ثبت کنید.</p>
        ...
        <form  wire:key="employee-form-{{ $formKey }}"   wire:submit.prevent="{{ $editMode ? 'update' : 'save' }}" class="grid grid-cols-1 md:grid-cols-2 mt-3 gap-4">
            ...
            <div class="md:col-span-2 flex flex-col md:flex-row gap-2">
                <button  wire:click="resetForm" type="button" class="w-full md:w-auto flex-1 bg-red-600 hover:bg-red-700 text-white font-semibold py-3 rounded-md transition">
                    لغو
                </button>
                <button  type="submit" class="w-full bg-blue-600 md:w-auto flex-1 hover:bg-blue-700 text-white font-semibold py-3 rounded-md transition">
                    ثبت
                </button>
            </div>
        </form>
            ...
            <div class="lg:hidden space-y-3 ">
                ...
                <div class="flex gap-2 flex-1 min-w-[100px]">
                    <div class="relative flex-1">
                        ...
                        <input type="text" wire:model.debounce.500ms="filter" class="p-1 pr-8 w-full  bg-[#0948EE]/20 rounded-xl" placeholder="فیلتر">
                    </div>
                    <div class="relative flex-1">
                        <input type="text" wire:model.debounce.500ms="search" class="p-1 w-full  bg-[#0948EE]/20  rounded-xl" placeholder="جستجو....">
                   ...
                   @if($employees && $employees instanceof \Illuminate\Pagination\LengthAwarePaginator && $employees->lastPage() > 1)
                    <div class="flex items-start justify-center mt-4 gap-1 rtl:space-x-reverse">
                        ...
                        @for ($page = 1; $page <= $employees->lastPage(); $page++)
                            <button  wire:click="gotoPage({{ $page }})"  class="w-7 h-7 rounded-md border text-xs font-medium  {{ $employees->currentPage() == $page      ? 'border-blue-500 bg-[#0948EE] text-white'  : 'border-transparent bg-[#0948EE]/60 text-white hover:bg-[#0948EE]' }}" >
                                {{ $page }}
                            </button>
                        @endfor
                        <button  wire:click="nextPage"  @disabled($employees->onLastPage())  class="w-7 h-7 rounded-md border border-gray-300 bg-[#0948EE]/60 hover:bg-[#0948EE] text-white disabled:opacity-50" >
                            ›
                        </button>
                    </div>
                    @endif
            ...
            <div class="bg-[#616161]/5 rounded-2xl shadow-xl sm:hidden block p-3">
                ...
                <div class="grid grid-cols-2 gap-5 text-sm ">
                    ...
                    <div class="flex flex-col items-center">
                        ...
                        <div class="text-gray-900 font-bold">{{ $employee->job }}</div>
                    </div>
                </div>
                <div class="flex justify-center items-center gap-2 mt-3">
                    <div class="flex  gap-3 mt-5">
                        <a wire:click="goToEdit({{ $employee->id }})"
