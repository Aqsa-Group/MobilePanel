<div class="max-w-full mx-auto ">
    ...
    <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-4 sm:p-6">
        <div class="mb-4 space-y-3">
            <div class="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
                <h3 class="font-semibold text-gray-700 text-right">تایید/بلاک ثبت دستگاه‌ها:</h3>
            ...
            <div class="grid grid-cols-3 gap-2">
                <button wire:click="$set('statusFilter','approved')" class="w-full h-10 px-3 py-2 rounded-lg text-sm flex items-center justify-center {{ $statusFilter === 'approved' ? 'bg-blue-800 text-white' : 'bg-gray-100' }}">تایید</button>
                <button wire:click="$set('statusFilter','blocked')" class="w-full h-10 px-3 py-2 rounded-lg text-sm flex items-center justify-center {{ $statusFilter === 'blocked' ? 'bg-blue-800 text-white' : 'bg-gray-100' }}">بلاک</button>
                <button wire:click="$set('statusFilter','pending')" class="w-full h-10 px-3 py-2 rounded-lg text-sm flex items-center justify-center {{ $statusFilter === 'pending' ? 'bg-blue-800 text-white' : 'bg-gray-100' }}">در انتظار</button>
            </div>
        </div>
                ...
                <tbody>
                    @forelse($registers as $row)
                        <tr wire:key="desktop-register-row-{{ $row->id }}-{{ $row->status }}" class="border-b align-top">
                            <td class="p-2 text-[9px] font-bold break-words whitespace-normal">{{ $row->id }}</td>
                            ...
                            <td class="p-2 text-[9px]">
                                ...
                                @if($row->status === 'blocked') <span class="inline-flex items-center px-2 py-0.5 rounded-full bg-red-100 text-red-800 text-[10px] font-semibold">بلاک</span> @endif
                ...
                <div wire:key="mobile-register-card-{{ $row->id }}-{{ $row->status }}" class="rounded-xl border border-gray-200 bg-white shadow-sm p-3">
                    <div class="grid grid-cols-2 gap-x-3 gap-y-2 text-xs">
                        <div class="min-w-0 text-center">
                            ...
                            <p class="font-semibold break-words text-center">{{ $row->id }}</p>
                        </div>
                        <div class="min-w-0 text-center">
                        ...
                        <div class="min-w-0 text-center">
                            ...
                            <p class="font-semibold break-words text-center">{{ $row->shop_name ?? '-' }}</p>
                        </div>
                        <div class="min-w-0 text-center">
                            <p class="text-gray-500">ثبت کننده</p>
                            <p class="font-semibold break-words text-center">{{ $row->submittedBy?->name ?? ('کاربر #' . $row->submitted_by_user_id) }}</p>
                        </div>
                        <div class="min-w-0 text-center">
    ...
    @if($showDetailModal)
        ...
        @endphp
        <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/45 p-3 sm:p-4 sm:pt-20">
            <div class="w-full max-w-5xl h-[420px] rounded-2xl border {{ $surfaceClass }} bg-white shadow-2xl overflow-hidden flex flex-col">
                ...
                <div class="p-4 sm:p-5 flex-1 overflow-y-auto space-y-4">
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 text-sm">
                        ...
                        <div class="rounded-lg border {{ $cardClass }} p-3">
                            ...
                            <textarea wire:model.defer="reviewNote" rows="2" class="w-full rounded-lg border px-2 py-1 text-xs text-gray-700 bg-white" placeholder="یادداشت مدیریت (اختیاری/دلیل بلاک)"></textarea>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-2">
                        ...
                        <button wire:click="reject" class="w-full px-4 py-2 rounded-lg bg-red-600 text-white text-sm">بلاک</button>
                    </div>
                </div>
            </div>
        </div>
