...
<body>
    ...
    @endif
    <header class="fixed top-0 inset-x-0 w-full h-[72px] bg-white z-[60] shadow shadow-[#0B35CC] px-3">
        <div class="w-full h-full lg:px-2">
            <div class="h-full flex items-center justify-between">
                <div class="flex items-center gap-2">
                    ...
                    <a href="" class="flex items-center px-1 text-[#002ED3] select-none" aria-label="نبض بازار دیجیتال">
                        <span class="font-bold text-lg md:text-3xl whitespace-nowrap">نبض بازار دیجیتال</span>
                    </a>
                </div>
                ...
                <div class="flex items-center gap-2 sm:gap-3">
                    ...
                    <button
                        id="adminDarkToggle"
                        type="button"
                        class="toggle-btn w-10 h-10 flex items-center justify-center rounded-full transition"
    ...
    <aside id="sidebar"
        class="fixed top-[72px] bottom-0 right-0 z-40 h-[calc(100vh-72px)] w-64
        bg-white shadow shadow-[#0B35CC]
        transform translate-x-full transition-transform duration-200 ease-out
        lg:sticky lg:top-0 lg:h-screen lg:translate-x-0 lg:flex lg:flex-col lg:w-52 px-2 pb-2 pt-0" aria-label="Sidebar">
        <nav id="menu" class="p-3 pt-2 h-full flex flex-col justify-start">
            <div class="space-y-1.5">
                <a  href="{{ route('admin2.dashboard') }}"  data-section="dashboard"
                    class="menu-btn group w-full  {{ request()->routeIs('admin2.dashboard') ? 'bg-[#0B35CC] text-white' : 'text-gray-700' }} text-right flex  gap-3 rounded-xl px-3 py-2 text-sm transition hover:bg-[#0B35CC] hover:text-white">
                ...
                <a  href="{{ route('admin2.users') }}" data-section="users"
                    class="menu-btn group w-full text-right flex {{ request()->routeIs('admin2.users') ? 'bg-[#0B35CC] text-white' : 'text-gray-700' }} gap-3 rounded-xl px-3 py-2 text-sm  transition hover:bg-[#0B35CC] hover:text-white">
                ...
                <a
                            ...
                            class="menu-btn group w-full text-right flex {{ request()->routeIs('admin2.device-list') ? 'bg-[#0B35CC] text-white' : 'text-gray-700' }} gap-3 rounded-xl px-3 py-2 text-sm  transition hover:bg-[#0B35CC] hover:text-white">
                ...
                @if(auth('admin2')->user()?->isAdmin())
                    <a
                        ...
                        class="menu-btn group w-full text-right flex gap-3 rounded-xl px-3 py-2 text-sm  transition hover:bg-[#0B35CC] hover:text-white"
                        >
    ...
    <script>
        ...
        const applyAdminTheme = (theme) => {
            ...
            if (adminDarkToggle) {
                adminDarkToggle.innerHTML = isDark
                    ? '<i class="fa-solid fa-sun"></i>'
                    : '<i class="fa-solid fa-moon"></i>';
                adminDarkToggle.setAttribute('aria-label', isDark ? 'حالت روشن' : 'حالت تاریک');
                adminDarkToggle.setAttribute('title', isDark ? 'تغییر به حالت روشن' : 'تغییر به حالت تاریک');
            }
        };

        let savedTheme = null;
        try {
            savedTheme = localStorage.getItem('admin_theme');
        } catch (e) {}
