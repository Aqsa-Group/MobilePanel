<div>
    <link href="https://unpkg.com/aos@2.3.1/dist/aos.css" rel="stylesheet">

    <div  class="grid grid-cols-1 lg:grid-cols-2  mt-2 ">

        <div data-aos="fade-left"

            data-aos-offset="300"
            data-aos-easing="ease-in-sine" class=" rounded-xl w-full shadow p-2 bg-gray-50">
            <h2 class="text-2xl text-black m-auto font-bold mb-2 text-center">ثبت دستگاه / مفقودی</h2>
            {{-- پیام موفقیت --}}
            @if (session()->has('success'))
                <div class="mb-4 p-3 bg-green-100 text-green-700 rounded-lg">
                    {{ session('success') }}
                </div>
            @else
                <p class="text-gray-500   text-center">لطفاً اطلاعات تان را وارد کنید</p>
            @endif

            <form wire:submit.prevent="submit" wire:key="{{ $formKey }}"  class="grid grid-cols-1 md:grid-cols-2 gap-4 p-5">
                <div class="flex flex-col">
                    <div class="relative">
                        <span class="absolute inset-y-0 left-3 flex items-center text-gray-400">
                            <svg width="22" height="22" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M12.1596 10.87C12.0596 10.86 11.9396 10.86 11.8296 10.87C9.44957 10.79 7.55957 8.84 7.55957 6.44C7.55957 3.99 9.53957 2 11.9996 2C14.4496 2 16.4396 3.99 16.4396 6.44C16.4296 8.84 14.5396 10.79 12.1596 10.87Z" stroke="#292D32" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
                            <path d="M7.15973 14.56C4.73973 16.18 4.73973 18.82 7.15973 20.43C9.90973 22.27 14.4197 22.27 17.1697 20.43C19.5897 18.81 19.5897 16.17 17.1697 14.56C14.4297 12.73 9.91973 12.73 7.15973 14.56Z" stroke="#292D32" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
                            </svg>

                        </span>
                        <input type="text" wire:model.defer="owner_full_name" placeholder="نام مالک"
                            class=" border rounded-lg px-4 py-4 w-full focus:outline-none focus:ring-2 focus:ring-blue-500"/>
                    </div>
                    @error('owner_full_name')
                        <span class="text-red-500 text-sm">{{ $message }}</span>
                    @enderror
                </div>
                <div class="flex flex-col">
                    <div class="relative">
                        <span class="absolute inset-y-0 left-3 flex items-center text-black-200">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none"
                            viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M3 5a2 2 0 012-2h3l2 5-2 1a11 11 0 005 5l1-2 5 2v3a2 2 0 01-2 2H16C8.82 19 3 13.18 3 6V5z"/>
                            </svg>
                        </span>
                        <input type="tel" dir="rtl" wire:model.defer="owner_phone" placeholder="شماره تماس"
                            class="border rounded-lg px-4 py-4 w-full"/>
                    </div>
                    @error('owner_phone')
                        <span class="text-red-500 text-sm">{{ $message }}</span>
                    @enderror
                </div>
                <div class="flex flex-col">
                    <div class="relative">
                        <span class="absolute inset-y-0 right-3 flex items-center text-gray-400">
                            <svg width="22" height="22" viewBox="0 0 24 24" fill="none"
                                xmlns="http://www.w3.org/2000/svg">
                                <path d="M9 22H15C20 22 22 20 22 15V9C22 4 20 2 15 2H9C4 2 2 4 2 9V15C2 20 4 22 9 22Z"
                                    stroke="#292D32" stroke-width="1.5"/>
                            </svg>
                        </span>
                        <input type="file"
                            wire:model="owner_photo"
                            class="border rounded-lg px-4 py-4 w-full" />
                    </div>

                    @error('owner_photo')
                        <span class="text-red-500 text-sm">{{ $message }}</span>
                    @enderror
                </div>

                <div class="flex flex-col">
                    <div class="relative">
                        <span class="absolute inset-y-0 left-3 flex items-center text-gray-400">
                            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M17 21H7C3 21 2 20 2 16V8C2 4 3 3 7 3H17C21 3 22 4 22 8V16C22 20 21 21 17 21Z" stroke="#292D32" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
                            <path d="M14 8H19" stroke="#292D32" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
                            <path d="M15 12H19" stroke="#292D32" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
                            <path d="M17 16H19" stroke="#292D32" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
                            <path d="M8.50043 11.2899C9.50007 11.2899 10.3104 10.4796 10.3104 9.47992C10.3104 8.48029 9.50007 7.66992 8.50043 7.66992C7.50079 7.66992 6.69043 8.48029 6.69043 9.47992C6.69043 10.4796 7.50079 11.2899 8.50043 11.2899Z" stroke="#292D32" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
                            <path d="M12 16.3298C11.86 14.8798 10.71 13.7398 9.26 13.6098C8.76 13.5598 8.25 13.5598 7.74 13.6098C6.29 13.7498 5.14 14.8798 5 16.3298" stroke="#292D32" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
                            </svg>

                        </span>
                        <input type="number" wire:model.defer="owner_national_id" placeholder="آیدی تذکره"
                            class="border rounded-lg px-4 py-4 w-full"/>
                    </div>
                    @error('owner_national_id')
                        <span class="text-red-500 text-sm">{{ $message }}</span>
                    @enderror
                </div>
                <div class="flex flex-col">
                    <div class="relative">
                        <span class="absolute inset-y-0 left-3 flex items-center text-gray-400">
                            <svg width="22" height="22" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M10 16.9498H6.21C2.84 16.9498 2 16.1098 2 12.7398V6.73979C2 3.36979 2.84 2.52979 6.21 2.52979H16.74C20.11 2.52979 20.95 3.36979 20.95 6.73979" stroke="#292D32" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
                            <path d="M10 21.4702V16.9502" stroke="#292D32" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
                            <path d="M2 12.9502H10" stroke="#292D32" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
                            <path d="M6.74023 21.4702H10.0002" stroke="#292D32" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
                            <path d="M22.0003 12.7998V18.5098C22.0003 20.8798 21.4103 21.4698 19.0403 21.4698H15.4903C13.1203 21.4698 12.5303 20.8798 12.5303 18.5098V12.7998C12.5303 10.4298 13.1203 9.83984 15.4903 9.83984H19.0403C21.4103 9.83984 22.0003 10.4298 22.0003 12.7998Z" stroke="#292D32" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
                            <path d="M17.2445 18.25H17.2535" stroke="#292D32" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                            </svg>

                        </span>
                        <input type="text" wire:model.defer="device_model" placeholder="مدل دستگاه"
                            class="border rounded-lg px-4 py-4 w-full"/>
                    </div>
                    @error('device_model')
                        <span class="text-red-500 text-sm">{{ $message }}</span>
                    @enderror
                </div>
                <div class="flex flex-col">
                    <div class="relative">
                        <span class="absolute inset-y-0 left-3 flex items-center text-gray-400">
                            <svg width="22" height="22" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M22 11V17C22 21 21 22 17 22H7C3 22 2 21 2 17V7C2 3 3 2 7 2H8.5C10 2 10.33 2.44 10.9 3.2L12.4 5.2C12.78 5.7 13 6 14 6H17C21 6 22 7 22 11Z" stroke="#292D32" stroke-width="1.5" stroke-miterlimit="10"/>
                            </svg>

                        </span>
                        <input type="file" wire:model="device_image" placeholder=" آپلود عکس دستگاه"
                            class="border rounded-lg px-4 py-4 w-full"/>
                    </div>
                    @error('device_image')
                        <span class="text-red-500 text-sm">{{ $message }}</span>
                    @enderror
                </div>
                <div class="flex flex-col">
                    <div class="relative">
                        <span class="absolute inset-y-0 left-3 flex items-center text-gray-400">
                            <svg width="22" height="22" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M3.00977 11.2202V15.7102C3.00977 20.2002 4.80977 22.0002 9.29977 22.0002H14.6898C19.1798 22.0002 20.9798 20.2002 20.9798 15.7102V11.2202" stroke="#292D32" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
                            <path d="M12.0005 12C13.8305 12 15.1805 10.51 15.0005 8.68L14.3405 2H9.67048L9.00048 8.68C8.82048 10.51 10.1705 12 12.0005 12Z" stroke="#292D32" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
                            <path d="M18.3098 12C20.3298 12 21.8098 10.36 21.6098 8.35L21.3298 5.6C20.9698 3 19.9698 2 17.3498 2H14.2998L14.9998 9.01C15.1698 10.66 16.6598 12 18.3098 12Z" stroke="#292D32" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
                            <path d="M5.64037 12C7.29037 12 8.78037 10.66 8.94037 9.01L9.16037 6.8L9.64037 2H6.59037C3.97037 2 2.97037 3 2.61037 5.6L2.34037 8.35C2.14037 10.36 3.62037 12 5.64037 12Z" stroke="#292D32" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
                            <path d="M12 17C10.33 17 9.5 17.83 9.5 19.5V22H14.5V19.5C14.5 17.83 13.67 17 12 17Z" stroke="#292D32" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
                            </svg>
                        </span>
                        <input type="text" wire:model.defer="store_name" placeholder="نام فروشگاه"
                        class="border rounded-lg px-4 py-4 w-full"/>
                    </div>
                    @error('store_name')
                        <span class="text-red-500 text-sm">{{ $message }}</span>
                    @enderror
                </div>
                <div class="flex flex-col">
                    <div class="relative">
                        <span class="absolute inset-y-0 left-3 flex items-center text-gray-400">
                            <svg width="22" height="22" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M22 5.7C22 7.4 21.2 9 19.7 9.9C18.4 10.7 16.5 11.5 14 11.8V5.7C14 3.7 15.7 2 17.7 2H18.2C20.3 2 22 3.7 22 5.7Z" stroke="#17191C" stroke-width="1.5" stroke-miterlimit="10"/>
                            <path d="M21.9996 8V18.3C21.9996 20.4 20.2996 22 18.2996 22H17.7996C15.6996 22 14.0996 20.3 14.0996 18.3V11.9C19.5996 11.2 21.9996 8 21.9996 8Z" stroke="#17191C" stroke-width="1.5" stroke-miterlimit="10"/>
                            <path d="M2 18.3002C2 16.6002 2.8 15.0002 4.3 14.1002C5.6 13.3002 7.5 12.5002 10 12.2002V18.3002C10 20.4002 8.3 22.0002 6.3 22.0002H5.7C3.7 22.0002 2 20.3002 2 18.3002Z" stroke="#17191C" stroke-width="1.5" stroke-miterlimit="10"/>
                            <path d="M2 16V5.7C2 3.7 3.7 2 5.7 2H6.2C8.3 2 10 3.7 10 5.7V12.1C4.4 12.8 2 16 2 16Z" stroke="#17191C" stroke-width="1.5" stroke-miterlimit="10"/>
                            <path d="M10 12.0999C10.6 11.9999 11.3 11.9999 12 11.9999C12.7 11.9999 13.4 11.9999 14 11.8999" stroke="#17191C" stroke-width="1.5" stroke-miterlimit="10"/>
                            </svg>
                        </span>
                        <input type="number" wire:model.defer="device_imei" placeholder="شماره IMEI دستگاه"
                            class="border rounded-lg px-4 py-4 w-full"/>
                    </div>
                    @error('device_imei')
                        <span class="text-red-500 text-sm">{{ $message }}</span>
                    @enderror
                </div>
                <div class="flex flex-col">
                    <div class="relative">
                    <select wire:model.defer="incident_type" class="border rounded-lg px-4 py-4 w-full">
                        <option value="">نوع حادثه</option>
                        <option value="lost">گم شده</option>
                        <option value="stolen">سرقت شده</option>
                    </select>
                    </div>
                    @error('incident_type')
                        <span class="text-red-500 text-sm">{{ $message }}</span>
                    @enderror
                </div>
                <div class="flex flex-col">
                    <div class="relative">
                    <span class="absolute inset-y-0 left-3 flex items-center text-gray-400">
                        <svg width="22" height="22" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path d="M11.9999 13.4299C13.723 13.4299 15.1199 12.0331 15.1199 10.3099C15.1199 8.58681 13.723 7.18994 11.9999 7.18994C10.2768 7.18994 8.87988 8.58681 8.87988 10.3099C8.87988 12.0331 10.2768 13.4299 11.9999 13.4299Z" stroke="#292D32" stroke-width="1.5"/>
                        <path d="M3.61971 8.49C5.58971 -0.169998 18.4197 -0.159997 20.3797 8.5C21.5297 13.58 18.3697 17.88 15.5997 20.54C13.5897 22.48 10.4097 22.48 8.38971 20.54C5.62971 17.88 2.46971 13.57 3.61971 8.49Z" stroke="#292D32" stroke-width="1.5"/>
                        </svg>
                    </span>
                    <input type="text" wire:model.defer="incident_location" placeholder="محل وقوع حادثه"
                        class="border rounded-lg px-4 py-4 w-full"/>
                    </div>
                    @error('incident_location')
                        <span class="text-red-500 text-sm">{{ $message }}</span>
                    @enderror
                </div>
                <div class="flex flex-col">
                    <div class="relative">
                    <textarea wire:model.defer="incident_description" placeholder="توضیحات بیشتر"
                        class="border rounded-lg px-4 py-4 w-full"></textarea>
                    </div>
                    @error('incident_description')
                        <span class="text-red-500 text-sm">{{ $message }}</span>
                    @enderror
                </div>
                <div class="flex flex-col">
                    <div class="relative">
                    <span class="absolute inset-y-0 left-3 flex items-center text-gray-400">
                        <i class="fas fa-date"></i>
                    </span>
                    <input type="datetime-local" wire:model.defer="incident_date" placeholder="تاریخ وقوع حادثه"
                        class="border rounded-lg px-4 py-4 w-full"/>
                    </div>
                    @error('incident_date')
                        <span class="text-red-500 text-sm">{{ $message }}</span>
                    @enderror
                </div>
                <div class="md:col-span-2 flex gap-4 mt-4">
                    <button type="button" wire:click='resetForm'
                        class="flex-1 bg-red-600 text-white px-4 py-4 rounded-lg hover:bg-red-700">
                        لغو
                    </button>
                    <button
                        wire:loading.attr="disabled" type="submit" class="flex-1 bg-blue-600 text-white px-4 py-4 rounded-lg hover:bg-blue-700">
                        <span wire:loading.remove>ثبت گزارش</span>
                        <span wire:loading>در حال ثبت...</span>
                    </button>
                </div>
            </form>
        </div>
        <div  data-aos="fade-right"
            data-aos-anchor="#example-anchor"
            data-aos-offset="500"
            data-aos-duration="500" class="  lg:flex   ">
            <img src="https://i.postimg.cc/QC4nPTSK/undraw-newsfeed-8ms9.png"
                alt="phone"
                class="h-[500px] w-full ">
        </div>

        <script src="https://unpkg.com/aos@2.3.1/dist/aos.js"></script>
        <script> AOS.init(); </script>
    </div>


</div>
